...
{
    return cml::pi_4 * x - x * (fabs(x) - 1) * (0.2447f + 0.0663f * fabs(x));
}
float FT::atan2(float y, float x)
{
    if (fabs(x) > fabs(y))
    {
        ...
        if (x > 0.f)
            ...
            return y > 0.f ? atan + cml::pi : atan - cml::pi;
    }
    else
    {
        float atan = FT::atan(x / y);
        if (x > 0.f)
            return y > 0.f ? cml::pi_2 - atan : -cml::pi_2 - atan;
        else
            return y > 0.f ? cml::pi_2 + atan : -cml::pi_2 + atan;
    }
}
...
float FT::cos(float angle)
{
    ...
    if (angle < cml::pi_2)
        return FT::cos_32s(angle);
    if (angle < cml::pi)
        return -FT::cos_32s(cml::pi - angle);
    if (angle < threehalfpi)
        return -FT::cos_32s(angle - cml::pi);
    return FT::cos_32s(twopi - angle);
}
float FT::sin(float angle)
{
    return FT::cos(cml::pi_2 - angle);
}
void FT::sincos(float angle, float *sin, float *cos)
{
    // clamp to the range 0..2pi
    angle = angle - floorf(angle * invtwopi) * twopi;
    float sinmultiplier = angle > 0.f && angle < cml::pi ? 1.f : -1.f;
    angle = angle > 0.f ? angle : -angle;
    ...
    if (angle < cml::pi)
    {
        *cos = -FT::cos_32s(cml::pi - angle);
        *sin = sinmultiplier * FT::sqrt(1.f - *cos * *cos);
        return;
    }
    if (angle < threehalfpi)
    {
        *cos = -FT::cos_32s(angle - cml::pi);
        *sin = sinmultiplier * FT::sqrt(1.f - *cos * *cos);
        return;
    }
...
__m128 FT::atan_ps(__m128 x)
{
    ...
    return _mm_sub_ps(_mm_mul_ps(_mm_set1_ps(cml::pi_4), x),
...
__m128 FT::atan2_ps(__m128 y, __m128 x)
{
    ...
    __m128 shift = _mm_set1_ps(cml::pi);
    shift = _mm_sub_ps(shift, _mm_andnot_ps(absxgreaterthanabsy, _mm_set1_ps(cml::pi_2))); // substract cml::pi_2 if absx<=absy
...
__m128 FT::cos_ps(__m128 angle)
{
    ...
    cosangle = _mm_xor_ps(cosangle, _mm_and_ps(_mm_cmpge_ps(angle, _mm_set1_ps(cml::pi_2)), _mm_xor_ps(cosangle, _mm_sub_ps(_mm_set1_ps(cml::pi), angle))));
    cosangle = _mm_xor_ps(cosangle, _mm_and_ps(_mm_cmpge_ps(angle, _mm_set1_ps(cml::pi)), SIGNMASK));
    ...
    result = _mm_xor_ps(result, _mm_and_ps(_mm_and_ps(_mm_cmpge_ps(angle, _mm_set1_ps(cml::pi_2)), _mm_cmplt_ps(angle, _mm_set1_ps(threehalfpi))), SIGNMASK));
    return result;
}
__m128 FT::sin_ps(__m128 angle)
{
    return FT::cos_ps(_mm_sub_ps(_mm_set1_ps(cml::pi_2), angle));
}
void FT::sincos_ps(__m128 angle, __m128 *sin, __m128 *cos)
{
    ...
    *cos = result;
    ...
    __m128 sinmultiplier = _mm_mul_ps(anglesign, _mm_or_ps(_mm_set1_ps(1.f), _mm_and_ps(_mm_cmpgt_ps(angle, _mm_set1_ps(cml::pi)), SIGNMASK)));
...
float FTA::cos(float angle)
{
    ...
    if (angle < cml::pi_2)
        return FTA::cos_52s(angle);
    if (angle < cml::pi)
        return -FTA::cos_52s(cml::pi - angle);
    if (angle < threehalfpi)
        return -FTA::cos_52s(angle - cml::pi);
    return FTA::cos_52s(twopi - angle);
}
float FTA::sin(float angle)
{
    return FTA::cos(cml::pi_2 - angle);
}
void FTA::sincos(float angle, float *sin, float *cos)
{
    ...
    if (angle < cml::pi)
    {
        *cos = -FTA::cos_52s(cml::pi - angle);
        *sin = sinmultiplier * FTA::sqrt(1.f - *cos * *cos);
        return;
    }
    if (angle < threehalfpi)
    {
        *cos = -FTA::cos_52s(angle - cml::pi);
        *sin = sinmultiplier * FTA::sqrt(1.f - *cos * *cos);
        return;
    }
...
{
    return FTA::cos_ps(_mm_sub_ps(_mm_set1_ps(cml::pi_2), angle));
}
...
void cml::trig::sincos(float angle, float *sin, float *cos){
    return FT::sincos(angle, sin, cos);
}
